...
if golem-cli deploy "$@"; then
    ...
    echo "  http://golem-rag-rust.localhost:9006"
    echo ""
    echo "Available endpoints:"
    echo "  POST /search                    - Basic semantic search"
    echo "  POST /search/filters            - Search with filters"
    echo "  POST /search/similar             - Find similar documents"
    echo "  GET  /documents/{id}           - Get document"
    echo "  GET  /documents                 - List documents"
    echo "  POST /s3/load/{namespace}      - Load S3 documents"
    echo "  GET  /s3/list/{namespace}       - List S3 documents"
