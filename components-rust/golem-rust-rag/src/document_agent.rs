...
pub trait DocumentAgent {
    ...
    fn list_documents(&self, filters: Option<DocumentFilters>, limit: Option<usize>) -> AgentResult<Vec<Document>>;
...
impl DocumentAgent for DocumentAgentImpl {
    ...
    fn get_document(&self, document_id: String) -> AgentResult<Option<Document>> {
        ...
        db_helper.load_document(&document_id)
    ...
    fn list_documents(&self, filters: Option<DocumentFilters>, limit: Option<usize>) -> AgentResult<Vec<Document>> {
        ...
        // Execute query
        let result = db_helper.connection.query(&sql_query, params)
    ...
    fn document_exists(&self, document_id: String) -> AgentResult<bool> {
        ...
        db_helper.document_exists_by_s3_key(&document_id)
...
impl DocumentAgentImpl {
    fn parse_document_from_row(&self, row: &PostgresDbRow) -> AgentResult<Document> {
        let id = try_match!(&row.values[0], PostgresDbValue::Text(id) => id.clone()).map_err(|_| "Invalid document ID type".to_string())?;
        let title = try_match!(&row.values[1], PostgresDbValue::Text(title) => title.clone()).map_err(|_| "Invalid title type".to_string())?;
        let content = try_match!(&row.values[2], PostgresDbValue::Text(content) => content.clone()).map_err(|_| "Invalid content type".to_string())?;
        let metadata_str = try_match!(&row.values[3], PostgresDbValue::Jsonb(metadata) => metadata.clone()).map_err(|_| "Invalid metadata type".to_string())?;
    ...
    fn parse_chunk_from_row(&self, row: &PostgresDbRow) -> AgentResult<DocumentChunk> {
        ...
        let document_id = try_match!(&row.values[1], PostgresDbValue::Text(document_id) => document_id.clone()).map_err(|_| "Invalid document ID type".to_string())?;
    ...
    fn build_document_list_query(&self, filters: Option<DocumentFilters>, limit: usize) -> AgentResult<(String, Vec<PostgresDbValue>)> {
