...
pub trait EmbeddingGeneratorAgent {
    ...
    fn generate_embeddings_for_document(&mut self, document_id: String) -> AgentResult<u32>;
    
    /// Get embedding status for a document
    fn get_embedding_status(&self, document_id: String) -> AgentResult<EmbeddingStatus>;
    ...
    fn generate_embeddings_for_documents(&mut self, document_ids: Vec<String>) -> AgentResult<u32>;
}
...
impl EmbeddingGeneratorAgent for EmbeddingGeneratorAgentImpl {
    ...
    fn generate_embeddings_for_document(&mut self, document_id: String) -> AgentResult<u32> {
        ...
        if let Err(e) = db_helper.update_embedding_status(&document_id, &EmbeddingStatus::InProgress) {
        ...
        for (chunk_index, chunk) in chunks.iter().enumerate() {
            match self.generate_and_store_embedding(&mut db_helper, &document_id, chunk_index as u32, chunk) {
                Ok(_) => embedding_count += 1,
                Err(e) => {
                    log::error!("Failed to generate embedding for chunk {}: {:?}", chunk_index, e);
        ...
        if let Err(e) = db_helper.update_embedding_status(&document_id, &EmbeddingStatus::Completed { chunk_count: embedding_count }) {
            log::warn!("Failed to update embedding status to completed: {:?}", e);
        }
        
        log::info!("Successfully generated {} embeddings for document: {}", embedding_count, document_id);
    ...
    fn generate_embeddings_for_documents(&mut self, document_ids: Vec<String>) -> AgentResult<u32> {
        ...
        for document_id in &document_ids {
            match self.generate_embeddings_for_document(document_id.clone()) {
        ...
        log::info!("Generated total of {} embeddings for {} documents", total_embeddings, document_ids.len());
        Ok(total_embeddings)
    }
}

impl EmbeddingGeneratorAgentImpl {
    fn chunk_document(&self, content: &str, config: &ChunkConfig) -> AgentResult<Vec<DocumentChunk>> {
        ...
        while chunk_start < content_len {
            ...
            let actual_chunk_end = if config.respect_sentences && chunk_end < content_len {
                self.find_sentence_boundary(&content_chars, chunk_start, chunk_end, config.chunk_size as usize)
            } else {
                chunk_end
            };
            ...
            let chunk_content: String = content_chars[chunk_start..final_chunk_end].iter().collect();
    ...
    fn find_sentence_boundary(&self, content: &[char], start: usize, end: usize, max_size: usize) -> usize {
    ...
    fn generate_and_store_embedding(
        &self,
        ...
        let embedding_vector = match &self.embedding_client {
            Some(client) => {
                match &self.embedding_provider {
                    Some(provider) => {
                        // For now, use mock embedding instead of async call
                        // TODO: Implement proper async embedding generation
                        log::debug!("Generating embedding for chunk: {}", &chunk.content);
                        EmbeddingClient::mock_embedding(&chunk.content, 768)
                    }
                    None => EmbeddingClient::mock_embedding(&chunk.content, 768),
                }
            }
            None => EmbeddingClient::mock_embedding(&chunk.content, 768),
        };
        ...
        // Store embedding
        db_helper.store_embedding(&embedding)
            .map_err(|e| format!("Failed to store embedding: {:?}", e))?;
        
        log::debug!("Generated and stored embedding for chunk {} of document {}", chunk_index, document_id);
        Ok(chunk_with_id.id)
    }
    ...
    fn mark_as_failed(&self, db_helper: &mut DatabaseHelper, document_id: &str, error: &str) {
        if let Err(e) = db_helper.update_embedding_status(
            document_id, 
            &EmbeddingStatus::Failed { error: error.to_string() }
        ) {
